...
class Archives
{
    ...
    function __construct($aid)
    {
        ...
        else
        {
			...
			if(!preg_match("#^http:\/\/#i", $this->addTableRow['body']))
			{
				$pattern_tmp = str_replace('/', '\/', $GLOBALS['cfg_cmspath']);
				if (!preg_match("#^{$pattern_tmp}#i", $this->addTableRow['body'])) {
					$pregRule = "/<img(.*?)src(\s*)=(\s*)[\'|\"]\/(.*?(?:[\.jpg|\.jpeg|\.png|\.gif|\.bmp|\.ico]))[\'|\"](.*?)[\/]?(\s*)>/i";
					$this->addTableRow['body'] = preg_replace($pregRule, '<img ${1} src="'.$GLOBALS['cfg_cmspath'].'/${4}" ${5} />', $this->addTableRow['body']);
				}
			}
			// ------end
        }//!error
    ...
    function ParAddTable()
    {
        ...
        if (isset($this->Fields['litpic']))
        {
            ...
            // 解决安装子目录之后，之前存储在数据表图片路径不对的问题 by wengxianhu
            if(!preg_match("#^http:\/\/#i", $this->Fields['litpic']))
            {
                $pattern_tmp = str_replace('/', '\/', $GLOBALS['cfg_cmspath']);
                if (!preg_match("#^{$pattern_tmp}#i", $this->Fields['litpic'])) {
                    $this->Fields['litpic'] = $GLOBALS['cfg_cmspath'].$this->Fields['litpic'];
                    $this->Fields['litpic'] = str_replace('//', '/', $this->Fields['litpic']);
                }
            }
			// ------end
